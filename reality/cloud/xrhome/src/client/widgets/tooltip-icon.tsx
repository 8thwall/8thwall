/* eslint-disable local-rules/hardcoded-copy -- Popup placement values are API tokens. */
...
const POPUP_PLACEMENT: Record<TooltipPosition, {
  position: 'top' | 'right' | 'bottom' | 'left'
  alignment: 'left' | 'center' | 'right'
}> = {
  'top left': {position: 'top', alignment: 'left'},
  'top right': {position: 'top', alignment: 'right'},
  'bottom right': {position: 'bottom', alignment: 'right'},
  'bottom left': {position: 'bottom', alignment: 'left'},
  'right center': {position: 'right', alignment: 'center'},
  'left center': {position: 'left', alignment: 'center'},
  'top center': {position: 'top', alignment: 'center'},
  'bottom center': {position: 'bottom', alignment: 'center'},
}
...
const TooltipIcon: React.FC<ITooltipIcon> = ({content, position = 'top left', wide = false}) => {
  const classes = useStyles()
  const {t} = useTranslation(['common'])
  const placement = POPUP_PLACEMENT[position]
  ...
  const trigger = (
    <button
      ...
      aria-label={t('button.learn_more', {ns: 'common'})}
    >
