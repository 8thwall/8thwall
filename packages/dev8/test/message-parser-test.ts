...
const makeMessage = (segmentWordLengths: number[]): Uint8Array => {
  const tableByteLength = 4 + segmentWordLengths.length * 4
  const alignedTableByteLength = tableByteLength + tableByteLength % 8
  const payloadByteLength = segmentWordLengths.reduce(
    (sum, words) => sum + words * 8,
    0
  )
...
const concatenate = (...arrays: Uint8Array[]): ArrayBuffer => {
...
describe('getMessageOffsets', () => {
  it('accepts an empty recording', () => {
    assert.deepEqual(getMessageOffsets(new ArrayBuffer(0)), [])
  })

  it('accepts a message with an empty segment', () => {
  ...
  it('finds boundaries for concatenated messages', () => {
    ...
    assert.deepEqual(getMessageOffsets(data), [
      ...
      {
        start: first.byteLength,
        end: first.byteLength + second.byteLength,
      },
    ])
  })

  it('rejects a truncated segment count', () => {
    assert.throws(
  ...
  it('rejects a truncated segment table', () => {
  ...
  it('rejects a truncated segment payload', () => {
  ...
  it('reports trailing invalid data at its byte offset', () => {
    const message = makeMessage([0])
    ...
    assert.throws(
      () => getMessageOffsets(data),
      new RegExp(
        `Invalid Cap'n Proto message at byte offset ${message.byteLength}`
      )
    )
  })
})
