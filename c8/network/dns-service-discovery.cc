...
void registerReply(
  ...
  auto &callback = *static_cast<DnsServiceDiscovery::RegisterCallback *>(context);
  callback();
}
...
void browseReply(
  ...
  if (errorCode != kDNSServiceErr_NoError) {
    C8_THROW("DNS Service Discovery Error code=" + std::to_string(errorCode));
  }
...
void getAddrInfoReply(
  ...
  switch (address->sa_family) {
    case AF_INET: {
      ...
      inet_ntop(AF_INET, &(ipv4->sin_addr), &serviceAddress.address[0], INET_ADDRSTRLEN);
      break;
    }
    case AF_INET6: {
      ...
      inet_ntop(AF_INET6, &(ipv6->sin6_addr), &serviceAddress.address[0], INET6_ADDRSTRLEN);
      break;
    }
    default: {
      C8_THROW("DNS Service Discovery unexpected family " + std::to_string(address->sa_family));
    }
  }

  auto &callback = *static_cast<DnsServiceDiscovery::GetAddrInfoCallback *>(context);
  bool moreResults = flags & kDNSServiceFlagsMoreComing;
...
DnsServiceDiscovery DnsServiceDiscovery::newRegisterRequest(
  const char *regtype, uint16_t port, FdEventListener &fdListener, RegisterCallback callback) {
  ...
  fdListener.addFdEvent(
    fd, EventFlag::READ | EventFlag::EDGE_TRIGGER | EventFlag::PERSIST, [ref]() {
  ...
  if (error != kDNSServiceErr_NoError) {
    ...
    C8_THROW("DNS Service Discovery Error code=" + std::to_string(error));
  }
...
DnsServiceDiscovery DnsServiceDiscovery::newResolveRequest(
  const DnsServiceAd &serviceAd, FdEventListener &fdListener, ResolveCallback callback) {
  // Move the callback onto the heap.
  std::unique_ptr<decltype(callback)> heapCallback(new decltype(callback)(std::move(callback)));
...
DnsServiceDiscovery DnsServiceDiscovery::newGetAddrInfoRequest(
  const DnsServiceInfo &serviceInfo, FdEventListener &fdListener, GetAddrInfoCallback callback) {
  // Move the callback onto the heap.
  std::unique_ptr<decltype(callback)> heapCallback(new decltype(callback)(std::move(callback)));
  ...
  auto fd = DNSServiceRefSockFD(ref);
  fdListener.addFdEvent(
    DNSServiceRefSockFD(ref), EventFlag::READ | EventFlag::EDGE_TRIGGER, [ref]() {
      DNSServiceProcessResult(ref);
    });
