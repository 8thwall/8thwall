...
#include "c8/pixels/draw2.h"
  ...
  void Figure::legend() { isDrawLegend_ = true; }
  ...
  void Figure::draw(RGBA8888PlanePixels out) {
    ...
    if (isDrawLegend_) {
      ...
      for (size_t i = 0; i < data_.size(); i++) {
        ...
        putText(
          name, {drawArea_.offsetX_ + xOffset, yOffset}, Color::OFF_WHITE, color, FONT_SIZE, out);
    ...
    Vector<float> yTickLocations =
      getYValLocations(drawArea_.paddingY_, height, out.rows(), minY_, maxY_, yTicks);
    ...
    for (int i = 0; i < data_.size(); i++) {
      ...
      putText(names_[i], {lastX + 2, lastY}, colors_[i], Color::OFF_WHITE, FONT_SIZE, out);
    }
  }
